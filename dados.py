...
def analisar_imagem_comprovante(arquivo_imagem):
    ...
    try:
        ...
        # Procura por data no formato dd/mm/aaaa ou dd/mm/aa
        data_match = re.search(r"(\d{2}/\d{2}/\d{2,4})", texto_extraido)
        # Procura pela palavra TOTAL, VALOR, or PAGAR, seguida opcionalmente por R$ e então pelo número
        valor_match = re.search(r"(?:TOTAL|Valor|PAGAR)\s*R?\$\s*([\d,]+\.?\d*)", texto_extraido, re.IGNORECASE)

        # Se encontrar uma data, usa-a. Senão, usa a data de hoje.
        if data_match:
            data_compra = data_match.group(1)
            # Converte data de 2 dígitos para 4 dígitos (ex: 25 -> 2025)
            ...
            print("AVISO: Nenhuma data encontrada no comprovante, usando a data de hoje.")
